...
static void Reset_Handler(void) {
  ...
  for (dst = &_sbss; dst < &_ebss; ++dst)
    *dst = 0;
  /* Jednostkę zmiennoprzecinkową trzeba włączyć przed wywołaniem
  funkcji main, bo optymalizator potrafi emitować instrukcje
  zmiennoprzecinkowe w zupełnie nieprzewidywalnych miejscach. */
  FPUenable();
